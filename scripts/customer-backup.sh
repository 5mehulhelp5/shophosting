...
# Usage: ./customer-backup.sh <customer_id> [backup_type]
# backup_type: db, files, or both (default: both)

set -euo pipefail

if [ -z "$1" ]; then
    echo "Usage: $0 <customer_id> [backup_type]"
    exit 1
fi
...
if [[ ! "$BACKUP_TYPE" =~ ^(db|files|both)$ ]]; then
    echo "ERROR: Invalid backup_type. Must be: db, files, or both"
    exit 1
fi
...
# Backup database if requested (db or both)
if [[ "$BACKUP_TYPE" == "db" || "$BACKUP_TYPE" == "both" ]]; then
    ...
    log "Skipping database backup (type: ${BACKUP_TYPE})"
fi
...
# Build list of directories to backup based on type
CUSTOMER_BACKUP_DIRS=""
if [[ "$BACKUP_TYPE" == "db" || "$BACKUP_TYPE" == "both" ]]; then
    # Only include DB dump dir if a database was actually dumped
    if [ -f "${DB_DUMP_DIR}/database.sql" ]; then
        CUSTOMER_BACKUP_DIRS="${DB_DUMP_DIR}"
    fi
fi
if [[ "$BACKUP_TYPE" == "files" || "$BACKUP_TYPE" == "both" ]]; then
    CUSTOMER_BACKUP_DIRS="${CUSTOMER_BACKUP_DIRS} ${CUSTOMER_DIR}"
fi

if [ -z "$CUSTOMER_BACKUP_DIRS" ]; then
    log "ERROR: No directories to backup"
    exit 1
fi

log "Backing up: ${CUSTOMER_BACKUP_DIRS}"
...
if [ ${PIPESTATUS[0]} -eq 0 ]; then
    ...
    # Output for backup_worker.py to parse
